...
const getStatusInfo = (status: Status): { Icon: React.FC<{className?: string}>, style: string, text: string } => {
    switch (status) {
        case Status.LISTENING:
            return { Icon: MicrophoneIcon, style: 'bg-wavy-gold-button text-black pulse', text: 'الاستماع...' };
        case Status.THINKING:
            return { Icon: BrainIcon, style: 'bg-wavy-gold-button text-black pulse-thinking', text: 'التفكير...' };
        case Status.SPEAKING:
            return { Icon: VolumeUpIcon, style: 'bg-wavy-gold-button text-black animate-pulse', text: 'التحدث...' };
        case Status.ERROR:
            return { Icon: XCircleIcon, style: 'bg-red-700 text-white', text: 'خطأ' };
        case Status.IDLE:
        default:
            // This case only happens briefly when the session is active but waiting for the next action
            return { Icon: MicrophoneIcon, style: 'bg-wavy-gold-button text-black', text: 'جاهز' };
    }
};
